// Enhanced Animation Utilities for StoryNest
...
export const characterFormAnimations = {
  ...
  validation: 'shake duration-500',
  success: 'bounce duration-600'
}
...
// Scroll-triggered animation observer utility
export class ScrollAnimationObserver {
      ...
      this.observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            const activeClass = this.elements.get(entry.target)
            if (entry.isIntersecting && activeClass) {
              entry.target.classList.add(...activeClass.split(' '))
            }
          })
        },
        {
  ...
  observe(element: Element, animationClass: string, activeClass: string) {
    ...
    // Store the active class for when element is visible
    this.elements.set(element, activeClass)
